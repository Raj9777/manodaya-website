  ...
  return (
    <section id="clinical-team" className="section-padding" style={{ backgroundColor: mode === 'full' ? '#FFFFFF' : '#FAFAFD' }}>
      <div className="container">
        ...
        {/* Full-Width Landscape Mode Rows Stack for Clinical Specialists */}
        <div 
          style={{
            display: 'flex',
        ...
        >
          {TEAM_MEMBERS.map((member) => (
            ...
            >
              {/* Landscape Layout Grid: Left Square Photo Column | Right Doctor Details */}
              <div 
                style={{
                  display: 'grid',
              ...
              >
                {/* Left Landscape Column: 1:1 Square View Doctor Photo */}
                ...
                {/* Right Landscape Column: Doctor Info, Bios & Special Interests */}
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <h3 style={{ fontSize: '1.9rem', fontWeight: 900, color: '#0E0E10', lineHeight: 1.2, marginBottom: '4px' }}>
                        {member.name}
                      </h3>

                      <div style={{ fontSize: '1rem', fontWeight: 800, color: '#8A4FFF' }}>
                        {member.role}
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '24px' }}>
                      {mode === 'full' ? (
                        <div style={{ fontSize: '0.969rem', color: '#475569', lineHeight: 1.65, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                          <p>{member.fullBioPara1}</p>
                          <p>{member.fullBioPara2}</p>
                        </div>
                      ) : (
                        <p style={{ fontSize: '0.969rem', color: '#475569', lineHeight: 1.65 }}>
                          {member.shortBio}
                        </p>
                      )}
                    </div>
                  ...
                  {/* Action Buttons */}
                  <div style={{ display: 'flex', gap: '12px', paddingTop: '18px', borderTop: '1px solid #F1F1F5' }}>
                    <button 
                      className="btn-black"
                      onClick={() => onOpenBooking(`Consultation with ${member.name}`)}
                      style={{ flex: 1, padding: '13px 20px', fontSize: '0.938rem' }}
                    >
                      <Calendar size={17} />
                    ...
                    {mode === 'short' && (
                      <button 
                        ...
                        style={{ padding: '13px 20px', fontSize: '0.938rem' }}
                      >
